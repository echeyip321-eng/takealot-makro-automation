...
class MakroFSNFinder:
    ...
    def search_makro(self, product_title: str):
        try:
            search_query = (
                (product_title or "")
                .replace("DH - ", "")
...
class MakroApi:
    ...
    def _request(self, method: str, endpoint: str, json_body=None):
        ...
        if 300 <= resp.status_code < 400:
            ...
            raise RuntimeError(
                "Makro API redirected to a different host. Blocked for safety"
            )
...
class ReviewQueue:
    ...
    def get_approved_items(self):
        ...
        try:
            ...
            for row in csv_data:
                ...
                sku = (row.get("Takealot SKU", "") or "").strip()
    ...
    def mark_as_listed(self, takealot_sku, listing_id):
        logger.info(
            f"Would mark {takealot_sku} as listed with Makro listing {listing_id}"
        )
...
def build_makro_listing(
    fsn: str, sku: str, price: float, location_id: str, inventory: int = 10
):
